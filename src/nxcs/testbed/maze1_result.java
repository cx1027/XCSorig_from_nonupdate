...
//				} else if (c != 'O' && c != 'T') {
	...
	public ActionPareto getReward(String state, int action) {
		...
		try {
			...
			if (x == 6 && y == 1) {
				reward.setPareto(new Qvector(-1, 20000));
				// resetPosition();
			}
...
//				action = -1;
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			String[] actionSelectionMethods = { "e-greedy" };// , "maxH",
																// "random"
			...
			params.weights = new double[][] { { 0, 1 }, { 1, 0 }, { 0.5, 0.5 } };
...
//                        nxcs.runIteration(finalStateCount, maze.getState(), weightSet, i);
						...
						System.out.println(
								String.format("######### begin to run of: Action:%s - Distance:%s - Trail#: %s ",
										actionSelectionMethod, weight, trailIndex));
						...
						while (finalStateCount < finalStateUpperBound) {
							...
							//print classifier every step to see why classfiers are same on 31 and 41
...
//								}
	...
	private void printOpenLocationClassifiers(int timestamp, maze1_result maze, NXCS nxcs, double[] weight) {
